...
import com.adobe.qe.toughday.internal.core.distributedtd.cluster.driver.requests.SlaveRequestProcessor;
...
public class RequestProcessorDispatcherTest {
    ...
    public void testMasterRequestProcessorIsReturnedForMaster() {
        ...
        Mockito.when(driverStateMock.getCurrentState()).then((Answer<DriverState.State>) invocationOnMock -> DriverState.State.MASTER);
    ...
    @Test
    public void testSlaveRequestProcessorIsReturnedForSlave() {
        Mockito.when(mockDriver.getDriverState()).then((Answer<DriverState>) invocationOnMocl -> driverStateMock);
        Mockito.when(driverStateMock.getCurrentState()).then((Answer<DriverState.State>) invocationOnMock -> DriverState.State.SLAVE);

        RequestProcessorDispatcher requestProcessorDispatcher = RequestProcessorDispatcher.getInstance();
        Assert.assertTrue(requestProcessorDispatcher.getRequestProcessor(mockDriver) instanceof SlaveRequestProcessor);
    }
